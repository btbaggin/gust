...
use super::{NativeSample, AudioDevice, SAMPLE_RATE, SoundError};
use winapi::STRUCT;
use winapi::RIDL;
use std::mem::size_of;
use winapi::{
...
RIDL! {#[uuid(0xb021_0783, 0x89cd, 0x11d0, 0xaf, 0x8, 0x0, 0xa0, 0xc9, 0x25, 0xcd, 0x16)]
    interface IDirectSoundNotify(IDirectSoundNotifyVtbl): IUnknown(IUnknownVtbl) {
        fn SetNotificationPositions(dwPositionNotifies : DWORD,pcPositionNotifies : PVOID,) -> HRESULT,
    }}
...
impl DirectSoundDevice {
    pub fn new(buffer_len_bytes: u32) -> Result<Self, SoundError> {
        unsafe {
...
fn check<S: Into<String>>(code: i32, message: S) -> Result<(), SoundError> {
    if code == DS_OK { Ok(()) } 
    else { Err(SoundError::InitializationFailure(message.into())) }
}
...
unsafe fn write(ds_buffer: *mut IDirectSoundBuffer, offset_bytes: u32, len_bytes: u32, data: &[NativeSample]) {
    ...
    (*ds_buffer).Lock(offset_bytes, len_bytes, &mut device_buffer, &mut size, 
...
impl AudioDevice for DirectSoundDevice {
    fn get_mixer(&mut self) -> super::MixContext {
        super::MixContext {
        ...
        loop {
            ...
            unsafe {
                ...
                match WaitForMultipleObjects(2, self.notify_points.as_ptr(), 0, INFINITE) {
                    WAIT_OBJECT_0 => write(
                        self.buffer, self.buffer_len_bytes,
                        self.buffer_len_bytes, &self.out_data,
                    ),
