...
unsafe fn solve_manifold(body_a: &RigidBody, body_b: &RigidBody) -> Manifold {
    ...
    match (&body_a.shape, &body_b.shape) {
        ...
        (CollisionShape::Circle(a), CollisionShape::Polygon(b)) => collision::circle_to_polygon(&mut m, entity_a, entity_b, &a, &b),
        (CollisionShape::Polygon(b), CollisionShape::Circle(a)) => collision::circle_to_polygon(&mut m, entity_b, entity_a, &a, &b),
...
fn integrate_forces(body: &mut RigidBody, delta_time: f32) {
	...
	body.velocity = body.velocity + (body.force * body.inverse_mass + GRAVITY) * (delta_time / 2.);
...
unsafe fn integrate_velocity(body: &mut RigidBody, delta_time: f32) {
	...
	entity.position = entity.position + body.velocity * delta_time;
...
pub unsafe fn step_physics(delta_time: f32) { //TODO scene
    let physics = unsafe { PHYSICS.as_mut().unwrap() };
	...
	for i in 0..PHYSICS_ITERATIONS {
		for c in &contacts {
            let a = unsafe { &mut *(bodies.get_unchecked_mut(c.body_a) as *mut _) };
            let b = unsafe { &mut *(bodies.get_unchecked_mut(c.body_b) as *mut _) };
